...
def run_simulation(layer_plain, layer_defect, current_layer_index, total_layers):
    ...
    output_noise = haadf.interpolate(0.05).gaussian_filter(0.3).poisson_noise(dose_per_area=1e7).array.T
    filename = f"{batch}_layer{current_layer_index}_total{total_layers}.npy"
    filename_noise = f"{batch}_layer{current_layer_index}_total{total_layers}_noise.npy"
    np.save(save_dir / filename, output)
    np.save(save_dir / filename_noise, output_noise)
    return output
